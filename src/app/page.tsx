...
export default function Home() {
  ...
  const [linkedProfile, setLinkedProfile] = useState<{ name?: string; email?: string; image?: string; provider?: string } | null>(null);
  ...
  useEffect(() => {
    ...
    // Check if user returned from OAuth — fetch linked profile
    fetch("/api/profile")
      .then((res) => res.json())
      .then((data) => {
        if (data.connected) {
          setLinkedProfile(data);
          if (data.provider === "linkedin") setSocial((prev) => ({ ...prev, linkedin: true }));
          if (data.provider === "facebook") setSocial((prev) => ({ ...prev, facebook: true }));
        }
      })
      .catch(() => {});
  }, []);
  ...
  const getAiInsight = () => {
    ...
    let msg = "";
    if (linkedProfile?.name) {
      msg += `Connected as ${linkedProfile.name}. `;
    }
    if (resumeData) {
      ...
      <div className="px-8 py-6">
        ...
        {activeTab === "setup" && (
          <div className="fade-in">
            ...
            <div className="grid grid-cols-3 gap-3 mb-0">
              <SocialCard name="LinkedIn" platform="linkedin" linked={social.linkedin} onClick={() => toggleSocial("linkedin")} color="rgba(10,102,194,0.2)" profileName={linkedProfile?.provider === "linkedin" ? linkedProfile.name : undefined} profileImage={linkedProfile?.provider === "linkedin" ? linkedProfile.image : undefined} icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="#0a66c2"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" /></svg>} />
        ...
        {activeTab === "paths" && (
          <div className="fade-in">
            ...
            {pathsData && (pathsData._roleMapping?.current.created || pathsData._roleMapping?.target.created || pathsData._pathSource === "ai" || pathsData._pathSource === "hybrid") && (
              <div className="rounded-lg p-3 mb-4 text-[14px] text-purple-300 space-y-1" style={{ background: "rgba(124,58,237,0.08)", border: "1px solid rgba(124,58,237,0.2)" }}>
                {pathsData._roleMapping?.current.created && (
                  <div><strong>&ldquo;{pathsData._roleMapping.current.input}&rdquo;</strong> — AI analyzed and added to career graph</div>
                )}
                {pathsData._roleMapping?.target.created && (
                  <div><strong>&ldquo;{pathsData._roleMapping.target.input}&rdquo;</strong> — AI analyzed and added to career graph</div>
                )}
                {pathsData._pathSource === "ai" && (
                  <div>Career paths generated by AI (no direct graph route found)</div>
                )}
                {pathsData._pathSource === "hybrid" && (
                  <div>Career paths enhanced with AI-suggested alternatives</div>
                )}
              </div>
            )}
...
function SocialCard({ name, linked, onClick, color, icon, profileName, profileImage }: {
  name: string; platform: string; linked: boolean; onClick: () => void; color: string;
  icon: React.ReactNode; profileName?: string; profileImage?: string;
}) {
  return (
    <div className={`social-card ${linked ? "linked" : ""}`} onClick={onClick}>
      {linked && profileImage ? (
        <img src={profileImage} alt="" className="w-9 h-9 rounded-[9px] flex-shrink-0 object-cover" />
      ) : (

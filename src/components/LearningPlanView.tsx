  ...
  if ((!plan || plan.length === 0) && summary) {
    return (
      <div className="rounded-[14px] p-5" style={{ border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.04)" }}>
        <div className="text-[13px] font-semibold text-blue-400 mb-2">Learning Plan</div>
        <div className="text-[12px] text-gray-400 leading-relaxed whitespace-pre-wrap">{summary}</div>
      </div>
    );
  }
    ...
    <div>
      {summary && (
        <div className="rounded-[14px] p-[18px] mb-5" style={{ border: "1px solid rgba(59,130,246,0.2)", background: "rgba(59,130,246,0.04)" }}>
          <div className="text-[13px] font-semibold text-blue-400 mb-2">Plan Overview</div>
          <div className="text-[12px] text-gray-400 leading-relaxed">{summary}</div>
        </div>
      )}
      ...
      <div className="timeline">
        {plan.map((week, idx) => (
          <div key={idx} className="relative mb-4 fade-in" style={{ animationDelay: `${idx * 0.06}s` }}>
            <div className="absolute -left-10 w-[26px] h-[26px] rounded-full flex items-center justify-center text-[11px] font-semibold font-mono text-purple-300"
            ...
            <div className="rounded-xl p-3.5" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
              <div className="text-[13px] font-semibold text-white mb-2">{week.focus}</div>

              <div className="flex flex-wrap gap-1.5 mb-2.5">
                {week.skills.map((skill, sIdx) => (
                  <span key={sIdx} className="px-2.5 py-0.5 rounded-full text-[10px] text-purple-400"
              ...
              {week.courses.map((course, cIdx) => (
                <div key={cIdx} className="text-[11px] text-gray-500 mb-1.5">
                  📖 {course.name} ({course.provider})
                </div>
              ))}

              {week.milestone && (
                <div className="text-[11px] text-green-400 italic mt-2">

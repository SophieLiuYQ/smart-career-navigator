...
function SkillBar({ name, importance, have }: { name: string; importance: number; have: boolean }) {
  return (
    <div className="flex items-center gap-3 mb-2">
      <span className={`text-[11px] w-4 flex-shrink-0 ${have ? "text-green-400" : "text-red-400"}`}>
        {have ? "✓" : "✗"}
      </span>
      <span className="text-[12px] text-gray-300 w-36 truncate">{name}</span>
      ...
      <span className="text-[10px] text-gray-500 w-8 text-right font-mono">{importance}</span>
    </div>
  );
}
    ...
    <div className="space-y-4 fade-in">
      ...
      <div className="rounded-[14px] p-5" style={{ border: "1px solid rgba(124,58,237,0.2)", background: "rgba(124,58,237,0.06)" }}>
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="text-[15px] font-semibold text-white">{data.title}</div>
            {data.onet_code && (
              <div className="text-[10px] text-gray-500 font-mono mt-0.5">O*NET {data.onet_code}</div>
            )}
          </div>
          {data.outlook?.bright && (
        ...
        <p className="text-[12px] text-gray-400 leading-relaxed mb-3">{data.description}</p>

        {/* Stats row */}
        <div className="flex gap-4 flex-wrap">
          {data.salary?.median && (
            <div className="px-3 py-2 rounded-lg" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="text-[10px] text-gray-500 uppercase tracking-wider">Median Salary</div>
              <div className="text-[14px] font-semibold text-green-400">{data.salary.median}</div>
            </div>
          )}
          {data.outlook && (
            <div className="px-3 py-2 rounded-lg" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="text-[10px] text-gray-500 uppercase tracking-wider">Growth</div>
              <div className="text-[14px] font-semibold text-blue-400">{data.outlook.growth}</div>
              <div className="text-[10px] text-gray-500">{data.outlook.description}</div>
            </div>
          )}
          {data.education && (
            <div className="px-3 py-2 rounded-lg" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="text-[10px] text-gray-500 uppercase tracking-wider">Education</div>
              <div className="text-[13px] font-medium text-gray-300">{data.education}</div>
            </div>
          )}
        </div>
      </div>
      ...
      {data.top_skills && data.top_skills.length > 0 && (
        <div className="rounded-[14px] p-5" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
          <div className="text-[13px] font-semibold text-white mb-3">Skills Required — Your Gap Analysis</div>
      ...
      {data.key_tasks && data.key_tasks.length > 0 && (
        <div className="rounded-[14px] p-5" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
          <div className="text-[13px] font-semibold text-white mb-3">Key Tasks</div>
          <ul className="space-y-2">
            {data.key_tasks.map((task, i) => (
              <li key={i} className="flex gap-2.5 text-[12px] text-gray-400 leading-relaxed">
      ...
      {data.hot_technologies && data.hot_technologies.length > 0 && (
        <div className="rounded-[14px] p-5" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
          <div className="text-[13px] font-semibold text-white mb-3">In-Demand Technologies</div>
      ...
      {data.related_roles && data.related_roles.length > 0 && (
        <div className="rounded-[14px] p-5" style={{ border: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
          <div className="text-[13px] font-semibold text-white mb-3">Related Roles to Consider</div>
          <div className="flex flex-wrap gap-2">
            {data.related_roles.map((role, i) => (
              <span key={i} className="px-3 py-1.5 rounded-lg text-[12px] font-medium"
